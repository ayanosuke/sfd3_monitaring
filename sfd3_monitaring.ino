...
#if 0         // Monitaring_Control()関数で処理するのでコメントアウト
// O4の処理
  if(State_O[4][1] == 0){
    digitalWrite(O4, LOW);
  } else {
...
//  S00:                  // S00:idle
  switch(state){
    case 0:
      if(State_O[4][1] == 1){ // MonitaringLED O4:ON
        state = 1;
//      goto S00;     // 100ms待たずに再度ステートマシーンに掛ける
      } 
      break;

    case 1: // スピードによる点灯消灯処理
        if(gSpeedRef <= startStep){
          digitalWrite(O4, LOW);               // 消灯
        } else {
          digitalWrite(O4, HIGH);              // 点灯
        }
        if(State_O[4][1] == 0){                // MonitaringLED O4:OFF
          digitalWrite(O4, LOW);               // 消灯
          state = 0; 
        }
        break;
...
{
  if(SpeedSteps == 28){               // 28stepで受信したときの処理
